...
class TextAudioLoaderMultiNSFsid(torch.utils.data.Dataset):
    ...
    def _filter(self):
        ...
        lengths = []
        for audiopath, text, pitch, pitchf, formants, coarse_formants, d_vector, dv in self.audiopaths_and_text:
            if self.min_text_len <= len(text) and len(text) <= self.max_text_len:
                audiopaths_and_text_new.append([audiopath, text, pitch, pitchf, formants, coarse_formants, d_vector, dv])
    ...
    def get_d_vector(self, d_vector):
    ...
    def get_audio_text_pair(self, audiopath_and_text):
        ...
        d_vector = audiopath_and_text[6]
        dv = audiopath_and_text[7]

        phone, pitch, pitchf, f1, f2, f3, f4, f5, cf1, cf2, cf3, cf4, cf5 = self.get_labels(phone, pitch, pitchf, formant, coarse_formant)
        spec, wav = self.get_audio(file)
        d_vector = self.get_d_vector(d_vector)
        dv = self.get_sid(dv)
        ...
        return (spec, wav, phone, pitch, pitchf, f1, f2, f3, f4, f5, cf1, cf2, cf3, cf4, cf5, d_vector, dv)
    ...
    def get_labels(self, phone, pitch, pitchf, formant, coarse_formant):
        ...
        f1, f2, f3, f4, f5 = np.load(formant)
        cf1, cf2, cf3, cf4, cf5 = np.load(coarse_formant)
        ...
        return phone, pitch, pitchf, f1, f2, f3, f4, f5, cf1, cf2, cf3, cf4, cf5
...
class TextAudioCollateMultiNSFsid:
    ...
    def __call__(self, batch):
        ...
        d_vector = torch.FloatTensor(len(batch), 256)
