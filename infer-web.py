...
def preprocess_dataset(trainset_dir, exp_dir, sr, n_p, spk_id):
    sr = sr_dict[sr]
    ...
    cmd = (
        config.python_cmd
        + " trainset_preprocess_pipeline_print.py %s %s %s %s/logs/%s %s "
        % (trainset_dir, sr, n_p, now_dir, exp_dir, spk_id)
        ...
        with gr.TabItem(i18n("训练")):
            ...
            with gr.Group():  # 暂时单人的, 后面支持最多4人的#数据处理
                ...
                with gr.Row():
                    ...
                    but1.click(
                        preprocess_dataset, [trainset_dir4, exp_dir1, sr2, np7, spk_id5], [info1]
                    )
            with gr.Group():
