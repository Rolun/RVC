...
sid=int(sys.argv[14])
crepe_hop_length=int(sys.argv[15])
print(sys.argv)
...
def vc_single(sid,input_audio,f0_up_key,f0_file,f0_method,file_index,index_rate):
    ...
    audio_opt=vc.pipeline2(hubert_model,net_g,audio,input_audio,times,f0_up_key,f0_method,file_index,index_rate,if_f0,filter_radius,tgt_sr,resample_sr,rms_mix_rate,version,protect,crepe_hop_length,f0_file=f0_file,sid=sid)
    print(times)
    return audio_opt
...
get_vc(model_path)
wav_opt=vc_single(sid,input_path,f0up_key,None,f0method,index_path,index_rate)
